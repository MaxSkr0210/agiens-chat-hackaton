...
export function getAuthToken(): string | null {
  ...
  if (typeof window !== 'undefined') return window.localStorage.getItem('agiens_token');
  return null;
}
...
async function request<T>(path: string, options?: RequestOptions): Promise<T> {
  const base = API_URL || (typeof window !== 'undefined' ? '' : 'http://localhost:3001');
...
export const api = {
  auth: {
    telegramLogin: (data: { id: number; first_name?: string; last_name?: string; username?: string; photo_url?: string; auth_date: number; hash: string }) =>
      request<{ token: string; accountId: string; channel: string; externalId: string }>('/api/auth/telegram', { method: 'POST', body: data }),
  },
  accounts: {
    me: () => request<AccountMeDto>('/api/accounts/me'),
    setMcp: (body: SetMcpBody) => request<void>('/api/accounts/me/mcp', { method: 'PATCH', body }),
  },
  agents: {
    list: () => request<AgentDto[]>('/api/agents'),
    getOne: (id: string) => request<AgentDto>(`/api/agents/${id}`),
    create: (body: Pick<AgentDto, 'name' | 'description' | 'systemPrompt'> & { icon?: string; supportedCategories?: string | null }) =>
      request<AgentDto>('/api/agents', { method: 'POST', body }),
    update: (id: string, body: Partial<Pick<AgentDto, 'name' | 'description' | 'icon' | 'systemPrompt' | 'modelId' | 'supportedCategories'>>) =>
  ...
  chats: {
    ...
    create: (body?: CreateChatBody) =>
      request<ChatSummaryDto>('/api/chats', { method: 'POST', body: body ?? {} }),
    getOne: (id: string) => request<ChatWithMessagesDto>(`/api/chats/${id}`),
    send: (id: string, message: string, modelId?: string | null, opts?: { signal?: AbortSignal; withVoice?: boolean }) =>
      request<SendMessageResponse>(`/api/chats/${id}/send`, {
        method: 'POST',
        body: { message, modelId: modelId ?? undefined, withVoice: opts?.withVoice ?? false },
        signal: opts?.signal,
      }),
    sendVoice: async (id: string, audioBlob: Blob, modelId?: string): Promise<SendVoiceResponse> => {
    ...
    setModel: (id: string, modelId: string) =>
      request<void>(`/api/chats/${id}/model`, { method: 'POST', body: { modelId } }),
    setAgent: (id: string, agentId: string) =>
      request<void>(`/api/chats/${id}/agent`, { method: 'PATCH', body: { agentId } }),
    getTicket: (chatId: string) => request<TicketDto | null>(`/api/chats/${chatId}/ticket`),
  },
  tickets: {
    list: (status?: string) =>
      request<TicketDto[]>(status ? `/api/tickets?status=${encodeURIComponent(status)}` : '/api/tickets'),
    getOne: (id: string) => request<TicketDto>(`/api/tickets/${id}`),
    update: (id: string, body: { status?: string; category?: string; assignedAgentId?: string | null; priority?: number }) =>
      request<TicketDto>(`/api/tickets/${id}`, { method: 'PATCH', body }),
    escalate: (id: string) => request<TicketDto>(`/api/tickets/${id}/escalate`, { method: 'PATCH' }),
  },
};
